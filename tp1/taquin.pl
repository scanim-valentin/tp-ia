...
%    heuristique1(U, H).  % au debut on utilise l'heuristique 1 
   ...
   %trouver le nombre de différence entre deux lignes
   nb_diff_1D([],[],0).
   nb_diff_1D([A|A_tl],[A|B_tl], N) :- nb_diff_1D(A_tl,B_tl, N).
   nb_diff_1D([A|A_tl],[B|B_tl], N) :-
         A \= B ,
         nb_diff_1D(A_tl,B_tl, M),
         N is M + 1.
   
   %nb de différence entre deux matrices
   nb_diff_2D([],[],0).
   nb_diff_2D([A|A_tl],[A|B_tl], N) :- nb_diff_1D(A_tl,B_tl, N).
   nb_diff_2D([A|A_tl],[B|B_tl], N) :-
         A \= B ,
         nb_diff_2D(A_tl,B_tl, M),
         nb_diff_1D(A,B,N1D),
         N is M + N1D.
   
   %nb de différence entre la matrice de fin et U
    heuristique1(U, H) :- 
      final_state(Fin),
      nb_diff_2D(U, Fin, H).
...
%initial_state(Ini), coordonnees(X,Y,Ini,a).
